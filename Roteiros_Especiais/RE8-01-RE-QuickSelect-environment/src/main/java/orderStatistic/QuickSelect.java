...
public class QuickSelect<T extends Comparable<T>> {
	...
	private T quickSort(T[] array, int leftIndex, int rightIndex, int k) {
		if(leftIndex <= rightIndex) {
			...
			if(indexPivot < k - 1) {
				return quickSort(array, indexPivot + 1, rightIndex, k);
			} else {
				return quickSort(array, leftIndex, indexPivot - 1, k);
			}
		}
		return null;
	...
	private int partition(T[] array, int leftIndex, int rightIndex) {
		medianPivot(array, leftIndex, rightIndex);
	...
	private void medianPivot(T[] array, int leftIndex, int rightIndex) {
