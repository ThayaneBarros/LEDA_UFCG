...
public class HeapImpl<T extends Comparable<T>> implements Heap<T> {
	...
	private void heapify(int position) {
		if(!this.isLeaf(position) || this.isValidIndex(position)){
			int maxIndex = maxIndex(position, this.left(position), this.right(position));
			
			if(maxIndex != position) {
				Util.swap(this.heap, position, maxIndex);
				this.heapify(maxIndex);
			}
		}
	}

	private int maxIndex(int position, int left, int right) {
		int out = position;
		
		if(this.isValidIndex(left) && this.comparator.compare(this.heap[position], this.heap[left]) < 0) {
			out = left;
		}
		
		if(this.isValidIndex(right) && this.comparator.compare(this.heap[out], this.heap[right]) < 0) {
			out = right;
		}
	...
	private boolean isLeaf(int position) {
		return position > this.parent(this.index) && position <= this.index;
	}
