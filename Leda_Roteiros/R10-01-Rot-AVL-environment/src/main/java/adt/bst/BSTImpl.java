...
public class BSTImpl<T extends Comparable<T>> implements BST<T> {
	...
	protected int recursiveHeight(BSTNode<T> current) {
		...
		if(element != null) {
			...
			if(!nodeElement.isEmpty()) {
				if(!nodeElement.getRight().isEmpty()) {
					out = this.recursiveMinimum((BSTNode<T>) nodeElement.getRight());
				} else {
					out = recursiveSucessor(nodeElement, element);
				}
			}
		}
		return out;
		...
		if(element != null) {
			...
			if(!nodeElement.isEmpty()) {
				if(!nodeElement.getLeft().isEmpty()) {
					out = this.recursiveMaximum((BSTNode<T>) nodeElement.getLeft());
				} else {
					out = recursivePredecessor(nodeElement, element);
				}
			}
		}
		return out;
